...
After installing DLS, you can find the UUID of your DLS in the [admin portal](https://www.dynamsoft.com/license-/docs/selfhosting/managedls.html?ver=2.2.19#log-in-dls-management-portal) .
...
* Mobile Edition and Embedded Edition: A UUID represents a device with the same CPU ID, OS ID and MAC address.
...
For editions other than the JavaScript edition, the UUID is generated by default via Soft Binding: the gathered device information is combined with a *random number* to generate the UUID. And this random number is only cached on the device and not sent anywhere. This means
...
Another option is to generate UUID via "Hard Binding".
